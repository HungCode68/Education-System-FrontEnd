...
import { AttendanceService, AttendanceDto } from '../../../teacher/services/attendance.service';
...
export class StudentScheduleComponent implements OnInit {
  ...
  fetchTimetable() {
    ...
    this.scheduleService.getMyStudentTimetable(this.startDateStr(), this.endDateStr()).subscribe({
      next: (res: any[]) => {
        const events = res || [];
  ...
  navigateToClass(classId: number | string, event?: Event) {
    ...
    if (classId) {
      ...
      this.router.navigate(['/student/classes', classId]);
    }
  }
