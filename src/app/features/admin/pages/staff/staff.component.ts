...
import { Staff, StaffType, StaffContractType } from '../../models/staff.model';
...
export class StaffComponent implements OnInit {
  ...
  loadData() {
    ...
    const typeFilter = this.typeFilterControl.value || undefined;
    const deptIdFilter = this.deptFilterControl.value || undefined;
    ...
    if (deptIdFilter) {
      this.staffService.getByDepartment(deptIdFilter, this.currentPage() - 1, this.pageSize()).subscribe({
        next: (res) => {
          ...
          if (typeFilter) {
            list = list.filter(s => s.staffType === typeFilter);
          }
